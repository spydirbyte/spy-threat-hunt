...
(function () {
  ...
  function initReports() {
    ...
    $("#btn-gen-exec").addEventListener("click", async () => {
      ...
      $("#exec-report-actions").classList.remove("hidden");
    ...
    $("#btn-gen-analyst").addEventListener("click", async () => {
      ...
      const rep = await res.json();
      if (!res.ok) { toast(rep.error || "Report generation failed.", true); return; }
      lastAnalystReport = rep;
      renderAnalystReport(rep);
      $("#analyst-report-actions").classList.remove("hidden");
      btn.textContent = "↻ Regenerate Analyst Report";
    });

    $("#btn-load-sources").addEventListener("click", loadSources);

    $("#btn-copy-exec").addEventListener("click", () => copyReport(lastExecReport, "exec", $("#btn-copy-exec")));
    $("#btn-copy-analyst").addEventListener("click", () => copyReport(lastAnalystReport, "analyst", $("#btn-copy-analyst")));
    $("#btn-download-exec").addEventListener("click", () => downloadReport(lastExecReport, "exec"));
    $("#btn-download-analyst").addEventListener("click", () => downloadReport(lastAnalystReport, "analyst"));
  }

  let lastExecReport = null;
  let lastAnalystReport = null;

  function slugify(s) {
    return (s || "report").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
  }

  function reportToMarkdown(rep, kind) {
    if (!rep) return "";
    const date = (rep.generatedAt || "").slice(0, 19).replace("T", " ") + " UTC";
    const lines = [];

    if (kind === "exec") {
      lines.push(`# ${rep.huntName}`);
      lines.push(`**Executive Threat Intelligence Brief** — generated ${date}\n`);
      lines.push(`**Severity:** ${rep.severity.toUpperCase()}  `);
      lines.push(`**Total indicators:** ${rep.totalIOCs}  `);
      lines.push(`**Confirmed malicious:** ${rep.criticalCount}\n`);
      lines.push(`## Business Impact`);
      lines.push(rep.businessImpact.estimatedImpact + "\n");
      lines.push(`- Users at risk: ${rep.businessImpact.usersAtRisk}`);
      lines.push(`- External communications observed: ${rep.businessImpact.externalCommunications ? "Yes" : "No"}`);
      lines.push(`- Geographic spread: ${rep.businessImpact.geographicSpread.join(", ") || "Unknown"}\n`);
      lines.push(`## Timeline`);
      rep.timeline.slice(0, 10).forEach(t => {
        lines.push(`- ${(t.timestamp || "").slice(0, 19).replace("T", " ")} — ${t.event} (${t.severity})`);
      });
      lines.push("");
      lines.push(`## Recommendations`);
      rep.recommendations.forEach(r => lines.push(`- ${r}`));
      if (rep.sources && rep.sources.length) {
        lines.push("");
        lines.push(`## Sources`);
        rep.sources.forEach(s => lines.push(`- **${s.ref_id}** — ${s.label} (${s.ioc_count} IOC${s.ioc_count === 1 ? "" : "s"})`));
      }
    } else {
      lines.push(`# ${rep.huntName}`);
      lines.push(`**Analyst Report** — generated ${date}\n`);
      lines.push(`Enrichment: ${rep.enrichmentSummary.enriched} enriched / ${rep.enrichmentSummary.pending} pending\n`);
      lines.push(`## Indicators by Type`);
      Object.entries(rep.iocsByType).forEach(([t, arr]) => lines.push(`- ${t}: ${arr.length}`));
      lines.push("");
      lines.push(`## Threat Actor Hypotheses`);
      rep.threatActorHypotheses.forEach(h => lines.push(`- ${h}`));
      lines.push("");
      lines.push(`## Detection Opportunities`);
      rep.detectionOpportunities.forEach(d => lines.push(`- ${d}`));
      lines.push("");
      lines.push(`## Hunt Queries`);
      lines.push(`${rep.huntQueries.length} ready-to-run queries generated across Splunk / Sigma / KQL — see Hunt Forge tab for full text.`);
      if (rep.sources && rep.sources.length) {
        lines.push("");
        lines.push(`## Sources`);
        rep.sources.forEach(s => lines.push(`- **${s.ref_id}** — ${s.label} (${s.ioc_count} IOC${s.ioc_count === 1 ? "" : "s"})`));
    ...
    lines.push("");
    lines.push(`---`);
    lines.push(`*Generated by SPY-THREAT-HUNT V2*`);
    return lines.join("\n");
  }

  function copyReport(rep, kind, btn) {
    if (!rep) { toast("Generate the report first.", true); return; }
    const md = reportToMarkdown(rep, kind);
    navigator.clipboard.writeText(md).then(() => {
      const original = btn.textContent;
      btn.textContent = "Copied ✓";
      toast("Report copied to clipboard — ready to paste into an email or doc.");
      setTimeout(() => { btn.textContent = original; }, 1600);
    }).catch(() => toast("Couldn't copy — your browser may be blocking clipboard access.", true));
  }

  function downloadReport(rep, kind) {
    if (!rep) { toast("Generate the report first.", true); return; }
    const md = reportToMarkdown(rep, kind);
    const filename = `${slugify(rep.huntName)}-${kind}-report.md`;
